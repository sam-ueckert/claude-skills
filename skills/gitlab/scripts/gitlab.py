#!/usr/bin/env python3
"""
gitlab.py — GitLab API client for agent skills.
...
def cmd_push(args):
    ...
    if status.stdout.strip():
        subprocess.run(["git", "commit", "-m", "push via agent gitlab skill"], check=True)
...
def main():
    parser = argparse.ArgumentParser(description="GitLab API client for agent skills")
