#!/usr/bin/env python3
"""
github.py — GitHub API client for agent skills.
...
def cmd_push(args):
    ...
    if status.stdout.strip():
        subprocess.run(["git", "commit", "-m", "push via agent github skill"], check=True)
...
def main():
    parser = argparse.ArgumentParser(description="GitHub API client for agent skills")
