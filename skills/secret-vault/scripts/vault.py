...
def resolve_key() -> bytes:
    ...
    if tier == "env":
        ...
        if key:
            return key
        print("ERROR: VAULT_KEY environment variable not set.")
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(description="Agent Secret Vault")
    ...
    p_init.add_argument("--env", action="store_true", help="Use VAULT_KEY env var")
